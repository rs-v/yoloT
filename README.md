...
每当 YOLO11 在当前帧中出现**首次出现**的跟踪目标（即当前帧含有之前未保存过的跟踪 ID）时，脚本会自动将带有标注框和标签的帧保存为 JPEG 图片，文件名包含检测时间戳。规则如下：

- **不带跟踪 ID 的检测结果不保存**：仅保存具有跟踪 ID（rid）的目标。
- **相同 ID 只保存一张**：同一跟踪 ID 在整个运行周期内只保存一张图片，后续帧中再次出现时不再重复保存。
...
- 命令格式：`S xxxxx;`
- `xxxxx` 为 5 位零填充的故障类别 ID（例如类别 12 会发送 `S 00012;`）
...
- 🕐 **检测时间戳**和累计检测事件数
- 🖼️ **历史检测照片**：按时间倒序展示最近 50 次检测事件的照片；每个跟踪 ID（rid）只记录一次，不带跟踪 ID 的检测结果不会进入历史记录。Web 面板的记录规则与磁盘保存完全一致。
